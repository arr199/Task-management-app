  ...
  useEffect(() => {
    function handleClickOutSide(e: MouseEvent): void {
      ...
      if (taskContainer !== null && !(taskContainer?.contains(target) ?? false)) {
        ...
        openTaskFormOptions &&
        dotsMenu !== null &&
        !dotsMenu?.contains(target) &&
        target.id !== "3dots-open-icon" &&
        target.id !== "3dots-open-menu"
      ) {
  ...
  return (
    <>
      <motion.div
        ...
        id="task-form"
        className={`task z-20 rounded-lg w-[90vw] sm:max-w-[400px] sm:min-w-[320px]  break-words  flex flex-col absolute inset-0 m-auto sm:w-[500px] h-max min-h-[400px] bg-[#2B2C37] p-8 pointer-events-auto 
      ...
      >
        <div className="flex justify-between  break-words">
          ...
          {/* OPTIONS BUTTON */}
          <button
            id="3dots-open-button"
            onClick={() => {
              setOpenTaskFormOptions((old) => !old);
            }}
          >
            <BiDotsVerticalRounded id="3dots-open-icon" className="w-8 h-8 text-[#828FA3]" />{" "}
          </button>
        </div>
        <p className=" py-5 text-[0.8125rem] text-[#828FA3]">{formData?.description}</p>
        <span className="text-[0.8125rem] text-white font-bold">
          Subtasks ({[...formData.subtasks].filter((e) => e.isCompleted).length} of {formData.subtasks.length})
        </span>
        ...
        <button
          ...
          className="self-end justify-self-end mt-4 px-4 py-2 rounded bg-[#635FC7] text-white font-bold
        ...
        {openTaskFormOptions && (
          <div
            id="edit-delete-task-menu"
            className={` w-[180px] h-[80px] absolute gap-4 inset-0 ml-auto  mt-[75px] mr-[-40px] flex flex-col bg-[#20212C] p-4 rounded-lg text-[0.8125rem] items-start
           ${darkMode === "light" ? "bg-white" : ""}`}
          >
            <button
              onClick={handleEditTask}
              className={`text-[#828FA3]  hover:font-bold 
             ${darkMode === "light" ? " hover:text-black " : " hover:text-white "}`}
            >
              Edit Task
            </button>
            <button onClick={handleDeleteTask} className={"text-red-600 hover:text-red-400 hover:font-bold "}>
              Delete Task
            </button>
          </div>
        )}
