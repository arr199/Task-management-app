...
export function Login (): JSX.Element {
  ...
  return (
    ...
    <main className="grid place-items-center h-screen bg-gradient-to-r from-[#1b1a1a] to-[#212127] ">
      <form onChange={() => { setError('') }} onSubmit={handleSubmit} className=" flex flex-col gap-4 p-6 min-w-[320px] mb-20    ">

        <img src="/assets/logo-light.svg" alt='kanban logo' />
        <div className='relative py-6 w-full flex justify-center items-center'>
          <span className='absolute  text-red-500 py-4 text-[12px] text-center'>{error.length > 0 && error.replaceAll('Firebase:', '')} </span>
        </div>
        {/* EMAIL */}
        ...
        <div className="flex gap-2 relative    w-full">
          ...
          <input data-id='password-input' value={formData.password} onChange={(e) => { setFormData({ ...formData, password: e.target.value }) }}
        ...
        <button type='button' onClick={handleGoogleLogin}
          className='active:scale-95   shadow-[0px_0px_3px_2px_#5a52c7] flex items-center gap-4 justify-center w-full py-2    text-[14px] bg-transparent px-8 text-center text-white
                    rounded-r-full duration-300 transition-colors rounded-l-full  hover:bg-[#2b2828]    font-bold'><FcGoogle></FcGoogle>Google</button>
        <span className='text-[14px] text-center flex flex-col gap-2'>Don&apos;t have an account? <NavLink to="/signup" className='text-[#635FC7]'>Create one!</NavLink> </span>
      </form>
    </main>
