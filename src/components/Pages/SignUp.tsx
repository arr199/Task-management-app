...
export function SignUp (): JSX.Element {
  ...
  return (
    ...
    <main className=" grid place-items-center h-screen bg-gradient-to-r from-[#1b1a1a] to-[#212127] ">
      <form onChange={() => { setError('') }} onSubmit={handleSubmit} className=" flex flex-col gap-4 p-6 min-w-[320px] mb-20    " action="">
        <img src="/assets/logo-light.svg" alt='kanban logo' />
        <div className='relative py-6 flex justify-center items-center w-full'>
          <span className='absolute text-red-500  text-[12px] text-center'>{error.replaceAll('Firebase:', '')} </span>
        </div>
        {/* EMAIL */}
        ...
        {/* CREATE ACCOUNT */}
        <button disabled={formData.email.replace(/\s+/g, '') === '' || formData.password.replace(/\s+/g, '') === ''}
          className='disabled:bg-[#8e8cd8] mt-2 active:scale-95   shadow-[0px_0px_6px_2px_#5a52c7] bg-[#635FC7] font-bold hover:bg-[#908eca]   w-full self-center gap-4 py-2  text-[14px] px-8 text-center text-white
                    rounded-r-full rounded-l-full duration-300 transition-colors '>Create Account</button>
        {/* GO BACK TO LOGIN */}
        <span className='text-[14px] text-center flex flex-col gap-2'>Already have an account ? <NavLink to="/login" className='text-[#635FC7]'>Sign in!</NavLink> </span>
      </form>
    </main>
