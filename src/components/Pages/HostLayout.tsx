...
import { AddBoardForm } from '../AbsoluteContainers/AddBoardForm'
import { AddTaskForm } from '../AbsoluteContainers/AddTaskForm'
import { EditBoardForm } from '../AbsoluteContainers/EditBoardForm'
import { AddColumnForm } from '../AbsoluteContainers/AddColumnForm'
import { AuthContext } from '../Auth/AuthProvider'
import { doc, getDoc } from 'firebase/firestore'
import { useFirebase } from '../../Utils/functions'
import { AnimatePresence } from 'framer-motion'
import { DeleteBoardForm } from '../AbsoluteContainers/DeleteBoardForm'
import { Board } from '../Layout/Board'
import { EditTaskForm } from '../AbsoluteContainers/EditTaskForm'
import { TaskForm } from '../AbsoluteContainers/TaskForm'
...
export function HostLayout (): JSX.Element {
  ...
  useEffect(() => {
    if (user !== null && user !== undefined) {
      ...
      getBoards().catch(err => { console.log('ERROR FETCHING INITIAL BOARDS : ', err) })
    }
  }, [])
  ...
  return (
    <>
      <GlobalContext.Provider value={{
        ...
        <section style={showNewTask ? { pointerEvents: 'none', opacity: '0.5' } : {}} id='main-page' className={' w-screen h-screen flex  '}>
          <LeftNavBar></LeftNavBar>
          <main className='flex flex-col  overflow-hidden w-full'>
            <Header setShowNewTask={setShowNewTask} boards={boards} setBoards={setBoards} setShowEditBoardForm={setShowEditBoardForm} ></Header>
            <Board></Board>
          </main>
        </section>
        {/* ABSOLUTE CONTAINERS */}
        <AnimatePresence mode='wait'>
          ...
          {showNewBoardForm && <AddBoardForm setShowNewBoardForm={setShowNewBoardForm} boards={boards} setBoards={setBoards} ></AddBoardForm>}
          {showEditBoardForm && <EditBoardForm boards={boards} setBoards={setBoards} setShowEditBoardForm={setShowEditBoardForm} ></EditBoardForm>}
          {showNewColumnForm && <AddColumnForm boards={boards} setBoards={setBoards} setShowNewColumnForm={setShowNewColumnForm}></AddColumnForm>}
