...
export function AddColumnForm ({ boards, setBoards, setShowNewColumnForm }: AddColumnFormProps): JSX.Element {
  ...
  function handleAddColumn (): void {
    ...
    setBoards(newBoards)
    setDoc(doc(db, 'users', user as string), { boards: newBoards }).catch(err => { console.log('ERROR ADDING COLUMN', err) })
  ...
  return (
    <motion.section id='new-column-form'
      ...
      <h1 className={` font-bold   
          ${darkMode === 'light' ? ' text-black ' : 'text-white'}`} >New Column</h1>
      <div className='relative'>
        <input
          ...
          className={`text-[0.8125rem] w-full bg-[#2B2C37] p-3 rounded border-[1px]  border-[#bdbaba] 
                  ${darkMode === 'light' ? ' bg-white text-black' : 'text-white'}`} placeholder='e.g   Pending'></input>
        <span className='text-red-500 absolute right-2 top-4 text-[10px] font-light'>{showValidationErrors(formData.name)}</span>
      </div>
      <button disabled={formData.name.replace(/\s+/g, '') === ''}
        onClick={handleAddColumn}
        className='disabled:bg-red-500 mt-5 hover:bg-[#A8A4FF] font-bold bg-[#635FC7] text-white text-[13px] rounded-3xl p-3'>Add Column</button>
    </motion.section>
