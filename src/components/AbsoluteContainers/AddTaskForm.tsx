...
export function AddTaskForm ({ showNewTask, setShowNewTask }: AddTaskFormProps): JSX.Element {
  ...
  const [formData, setFormData] = useState<AddTaskForm>({ id: '', title: '', description: '', status: defaultStatusValue, subtasks: [{ title: '', id: nanoid(), isCompleted: false }] })
  ...
  function handleCreateTask (e: React.FormEvent): void {
    ...
    setBoards(newBoards)
    setDoc(doc(db, 'users', user as string), { boards: newBoards }).catch(err => { console.log('ERROR ADDING NEW TASK : ', err) })
    setShowNewTask(false)
  ...
  return (
    ...
    <motion.form onSubmit={handleCreateTask} id='add-task' className={`task gap-3 flex flex-col absolute inset-0 m-auto w-[90vw]  sm:w-max  sm:min-w-[450px] h-max min-h-[320px] px-5 py-8 rounded 
    ${darkMode === 'light' ? ' bg-[#ffffff] text-black ' : 'bg-[#2B2C37] text-white '}`}
      ...
      <div className='relative flex flex-col gap-1 text-[0.75rem] font-bold'>
        ...
        <input data-id="add-task-title"
          ...
          placeholder='e.g Take a coffee break'></input>
        <span className=' text-red-500 absolute right-6 top-8 text-[10px] font-light'>{showValidationErrors(formData.title)}</span>
      </div>
      {/* DESCRIPTION */}
      <div className='flex flex-col gap-1 text-[0.75rem] font-bold relative'>
        <label>Description</label>
        <textarea data-id="add-task-description"
        ...
        <span className=' text-red-500 absolute bottom-4 right-6 text-[10px] font-light'>{showValidationErrors(formData.description)}</span>
      </div>
      {/* SUBTASK */}
        ...
        ? <div className='flex flex-col gap-1 text-[0.75rem] font-bold w-full'>
          ...
          {formData.subtasks.map((subtask, index) =>
            <div className='flex items-center gap-2 w-full relative' key={subtask.id}>
              ...
              <span className=' text-red-500 absolute bottom-3 right-10 text-[10px] font-light'>{showValidationErrors(subtask.title)}</span>
            </div>
          )}
        </div>
      ...
      <button
        ...
        type='button' onClick={handleAddNewSubtask}
        className={` hover:text-white font-bold   text-[13px] rounded-3xl p-2 active:scale-95
        ${darkMode === 'light' ? ' bg-[#635FC7]  text-white hover:bg-[#A8A4FF] ' : ' bg-white text-[#635FC7] hover:bg-[#635FC7] '}`}>+ Add New Subtask </button>
      {/* STATUS */}
      ...
      <button disabled={formData.title.replace(/\s+/g, '') === '' || formData.description.replace(/\s+/g, '') === '' || formData.subtasks.some(e => e.title.replace(/\s+/g, '') === '') || formData.subtasks.length <= 0}
        className='disabled:bg-red-500 hover:bg-[#A8A4FF] font-bold bg-[#635FC7] text-white text-[13px] rounded-3xl p-2' >Create Task</button>
    </motion.form>
