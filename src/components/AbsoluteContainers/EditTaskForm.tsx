...
export function EditTaskForm (): JSX.Element {
  ...
  const [formData, setFormData] = useState<Task>({ id: currentTask?.id, title: currentTask?.title, status: currentTask?.status, description: currentTask?.description, subtasks: currentTask?.subtasks })
  ...
  function handleSaveChanges (e: React.FormEvent): void {
    ...
    setDoc(doc(db, 'users', user as string), { boards: newBoards }).catch(err => { console.log('ERROR EDITING TASK : ', err) })
  ...
  return (
    <motion.form draggable onDragStart={(e) => { e?.preventDefault() }}
      onSubmit={handleSaveChanges} id='edit-task-form'
    ...
    ${darkMode === 'light' ? ' bg-white  ' : ' bg-[#2B2C37] text-white '}`}
      ...
      {/* TITLE */}
      <h2 id='title' className='font-bold '>Edit Task</h2>
      <div className='relative flex flex-col gap-1 text-[0.75rem] font-bold'>
        ...
        <input
          ...
          value={formData.title} onChange={(e) => { setFormData(old => ({ ...old, title: e.target.value })) }}
          className={` hover:border-[#635FC7]  text-[0.8125rem] bg-[#2B2C37] p-2 rounded border-[1px] border-[#5f5f5f] 
          ${darkMode === 'light' ? ' bg-white text-black border-[#bdbaba]' : '  '} `} placeholder='e.g Take a coffee break'></input>
        <span className=' text-red-500 absolute top-8 right-5 text-[10px] font-light'>{showValidationErrors(formData.title)}</span>
      </div>
      {/* DESCRIPTION */}
      <div className='relative  flex flex-col gap-1 text-[0.75rem] font-bold'>
        ...
        <textarea
          ...
          value={formData.description} onChange={(e) => { setFormData(old => ({ ...old, description: e.target.value })) }}
        ...
        <span className=' text-red-500 absolute bottom-4 right-5 text-[10px] font-light'>{showValidationErrors(formData.description)}</span>
      </div>
      {/* SUBTASK */}
          ...
          {formData.subtasks.map((subtask, index) =>
            <div className='relative flex items-center gap-2 w-full' key={subtask.id}>
              < input
                ...
                ${darkMode === 'light' ? ' bg-white text-black border-[#bdbaba]' : '  '} `} placeholder='e.g Make a coffee'></input>
              <RxCross1 onClick={() => { handleRemoveSubtask(index) }}
                className='w-4 h-4 text-[#808080] hover:text-red-600 hover:scale-105 cursor-pointer'></RxCross1>
              <span className=' text-red-500 absolute  right-10 text-[10px] font-light'>{showValidationErrors(subtask.title)}</span>
            </div>
          )}
        </div>
        //  EMPTY SUBTASKS
      ...
      <button type='button' onClick={handleAddSubtask}
        ...
        ${darkMode === 'light' ? ' bg-[#635FC7]  text-white hover:bg-[#A8A4FF]   ' : ' bg-white text-[#635FC7] hover:bg-[#635FC7] hover:text-white '}`}>+ Add New Subtask </button>
      {/* STATUS */}
      ...
      {/* SAVE CHANGES */}
      <button disabled={formData.title.replace(/\s+/g, '') === '' || formData.description.replace(/\s+/g, '') === '' || formData?.subtasks?.some(e => e.title.replace(/\s+/g, '') === '') || formData.subtasks.length <= 0}
        className='disabled:bg-red-500 hover:bg-[#A8A4FF] font-bold bg-[#635FC7] text-white text-[13px] rounded-3xl p-2' >Save Changes</button>
    </motion.form>
  )
}
