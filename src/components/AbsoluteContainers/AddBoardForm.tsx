...
import { getColor, mainPointerEvents, removeOpacityClass, showValidationErrors, useFirebase } from '../../Utils/functions'
...
export function AddBoardForm ({ boards, setBoards, setShowNewBoardForm }: AddBoardFormProps): JSX.Element {
  ...
  function handleCreateBoard (e: React.FormEvent): void {
    ...
    setDoc(doc(db, 'users', user as string), { boards: newBoards }).catch((err) => { console.log('ERROR ADDING BOARDS:', err) })
  ...
  return (
    ...
    <motion.form id='add-board-form' onSubmit={handleCreateBoard}
    ...
    ${darkMode === 'light' ? ' text-black  bg-white' : 'bg-[#2B2C37] text-white '} ${smallScreenMediaQueries} `}
      ...
      <div className='flex flex-col gap-1 text-[0.75rem] font-bold py-4 relative'>
        ...
        <input
          ...
          value={formData.name} onChange={(e) => { setFormData(old => ({ ...old, name: e.target.value })) }}
          className={`hover:border-[#635FC7] text-[0.8125rem] bg-[#2B2C37] p-2 rounded border-[1px] 
          ${darkMode === 'light' ? ' bg-white text-black border-[#bdbaba]' : ' border-[#5f5f5f] bg-[#2B2C37] text-white '}`}
          placeholder='e.g. Web Design'></input>
        <span className=' text-red-500 absolute  top-12 right-5 text-[10px] font-light'>{showValidationErrors(formData.name)}</span>
        ...
        ? <div className='flex flex-col gap-1 text-[0.75rem] font-bold'>
          ...
          {formData.columns.map((column, index) =>
            <div className='flex items-center gap-2 w-full relative' key={column.id}>
              < input value={formData.columns[index].name} onChange={(e) => { handleInputChange(e, index) }}
                className={`hover:border-[#635FC7] text-[0.8125rem] w-full bg-[#2B2C37] p-2 rounded border-[1px] 
                
                ${darkMode === 'light' ? ' bg-white text-black border-[#bdbaba] ' : ' bg-[#2B2C37] text-white border-[#5f5f5f]  '}`}
                placeholder='e.g Done'></input>
              <button onClick={() => { handleDeleteColumn(index) }} > <RxCross1
                className='w-4 h-4 text-[#808080] hover:text-red-600 hover:scale-105 cursor-pointer'></RxCross1>
              </button>
              <span className=' text-red-500 absolute right-10 text-[10px] font-light'>{showValidationErrors(column.name)}</span>
      ...
      {/* ADD NEW COLUMN */}
      <button onClick={handleAddColumn} className={` font-bold  text-[13px] rounded-3xl p-2 my-2
      ${darkMode === 'light' ? ' bg-[#635FC7]  text-white hover:bg-[#A8A4FF] ' : ' bg-white text-[#635FC7] hover:bg-[#635FC7] hover:text-white '} `}
        type='button' >+ Add New Column</button>
      {/* CREATE NEW BOARD */}
      <button data-id="create-board-form-button" disabled={formData.name.replace(/\s+/g, '') === '' || formData.columns.some(e => e.name.replace(/\s+/g, '') === '') || formData.columns.length <= 0}
        onClick={handleCreateBoard} className='disabled:bg-red-500 hover:bg-[#A8A4FF] font-bold bg-[#635FC7] text-white text-[13px] rounded-3xl p-2' >Create New Board</button>
    </motion.form>
