...
export function EditBoardForm ({ boards, setBoards, setShowEditBoardForm }: EditBoardFormProps): JSX.Element {
  ...
  const InitialColumns = structuredClone(boards)[Number(board)].columns.map(e => ({ name: e.name, tasks: e.tasks, id: e.id, color: e.color }))
  ...
  function handleSaveChanges (e: React.FormEvent): void {
    ...
    setBoards(newBoards)
    setDoc(doc(db, 'users', user as string), { boards: newBoards }).catch(err => { console.log('ERORR EDITING BOARD : ', err) })
  ...
  return (
    <motion.form id='edit-board-form' onSubmit={handleSaveChanges} className={`task   gap-5 flex text-black flex-col absolute inset-0 m-auto w-[90vw] sm:w-max sm:min-w-[450px] h-max min-h-[320px]  px-5 py-8 rounded 
    ${darkMode === 'light' ? ' bg-white ' : ' bg-[#2B2C37] text-white '}`}
      ...
      <div className='relative flex flex-col gap-1 text-[0.75rem] font-bold'>
        ...
        <span className='text-red-500 absolute right-4 top-8 text-[10px] font-light'>{showValidationErrors(formData.name)}</span>
      </div>
      {/* COLUMNS */}
          ...
          {formData.columns.map((column, index) =>
            <div className='flex items-center gap-2 w-full' key={column.id}>
              < input
                ...
                value={formData.columns[index].name} onChange={(e) => { handleColumnChange(e, index) }}
                className={`hover:border-[#635FC7] text-[0.8125rem] w-full bg-[#2B2C37] p-2 rounded border-[1px] border-[#5f5f5f] 
                ${darkMode === 'light' ? ' bg-white text-black border-[#bdbaba]' : ' bg-[#2B2C37] text-white '}`}
                placeholder='e.g Doing'></input>
              <RxCross1 onClick={() => { handleRemoveColumn(column.id) }}
                className='w-4 h-4 text-[#808080] hover:text-red-600 hover:scale-105 cursor-pointer'></RxCross1>
              <span className='text-red-500 absolute right-14 text-[10px] font-light'>{showValidationErrors(column.name)}</span>
            </div>
          )}
        </div>
      ...
      <button onClick={handleAddColumn}
        ...
        ${darkMode === 'light' ? ' bg-[#635FC7]  text-white hover:bg-[#A8A4FF] ' : ' bg-white text-[#635FC7] hover:bg-[#635FC7] hover:text-white '}`} type="button">+ Add New Column </button>
      {/* SAVE CHANGES */}
      <button disabled={formData.name.replace(/\s+/g, '') === '' || formData.columns.some(e => e.name.replace(/\s+/g, '') === '') || formData.columns.length <= 0}
