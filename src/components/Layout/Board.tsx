...
export function Board (): JSX.Element {
  ...
  const [draggedElement, setDraggedElement] = useState<DraggedElement>({ column: null, task: null, targetColumn: null, targetTask: null })
  ...
  function handleDragOver (e: React.DragEvent, columnIndex: number, taskIndex: number): void {
    ...
    if (target.dataset.id === boards[Number(board)].columns[draggedElement.column as number].tasks[draggedElement.task as number].id) {
      return
    }
  ...
  function handleDropTask (e: React.DragEvent): void {
    ...
    if (targetTask === null && typeof targetColumn === 'number') {
      ...
      newBoard[Number(board)].columns[targetColumn].tasks.push({ ...newTask[0], id: nanoid(), status: newBoard[Number(board)].columns[targetColumn].name })
      setDraggedElement({ column: null, task: null, targetColumn: null, targetTask: null })
      setBoards(newBoard)
      setDoc(doc(db, 'users', user as string), { boards: newBoard }).catch(err => { console.log('ERORR DRAGGING TASK  : ', err) })
      return
    }
    ...
    newBoard[Number(board)].columns[targetColumn as number].tasks.splice(targetTask as number, 0, { ...newTask[0], id: nanoid(), status: newBoard[Number(board)].columns[targetColumn as number].name })
  ...
  return (
    <>
      {/* BOARD SECTION */}
      <section id='board' className={` transition-colors duration-300 flex text-[#828FA3] gap-6 overflow-auto w-full h-full ${showLeftNavbar ? ' md:pl-[340px] ' : ' md:pl-16  '} 
          ${darkMode === 'light' ? 'bg-[#F4F7FD]' : ''} `}>

        {/* columns */}
        {boards?.[Number(board)]?.columns?.map((column, columnIndex) =>
          <div data-id={column.id} onDragOver={(e) => { handleDragOverColumn(e, columnIndex) }} onDrop={handleDropTask} key={column.id} className='board-columns min-w-[300px]  flex items-center gap-4  flex-col p-4 ' >
            <h3 className='tracking-[2.4px] mb-4 font-semibold text-[12px] flex items-center gap-2 overflow-x-hidden   text-ellipsis max-w-[200px] '>
              <GoDotFill style={{ width: '20px', height: '20px', color: column.color }} />  {column.name} ({column.tasks.length}) </h3>
            {/* TASKS */}
            {column?.tasks.map((task, taskIndex) =>
              <div data-id={task.id} key={task.id} className=' dragged-element w-full'
                onDragOver={(e) => { handleDragOver(e, columnIndex, taskIndex) }}
                onDragStart={(e) => { handleDragStart(e, columnIndex, taskIndex) }}
        ...
        {/* ADD COLUMN */}
        {boards.length > 0 &&
          <div className={` transition-colors duration-300  h-[80%] my-auto rounded-lg flex flex-col items-center justify-center ml-auto   px-4   
              ${darkMode === 'light' ? '  bg-gradient-to-r from-[#E9EFFA] to-[#E9EFFA80]  ' : '  black-gradient '}`}>
            <div className='grid place-items-center min-w-[100px]'>
              <button onClick={handleAddNewColumn} className='hover:text-[#635FC7]  ml-auto z-20'>+ New Column</button>
            </div>
          </div>}
        {/* EMPTY BOARD */}
        { boards?.length <= 0 &&
          <div className='m-auto flex flex-col items-center gap-4'>
            ...
            <button onClick={() => { setShowNewBoardForm(true) }}
              className={` font-bold py-4 px-6 rounded-l-full rounded-r-full
                  ${darkMode === 'light' ? ' bg-[#635FC7] text-white  hover:bg-[#A8A4FF] ' : ' bg-[#635FC7] text-white hover:bg-white hover:text-[#635FC7] '}`} > Create Board</button>
          </div>}
