    ...
    <>
      <motion.section className="relative">
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              className={`absolute flex flex-col gap-2 mt-2  p-2  px-3 py-2 text-[0.8125rem] rounded w-full items-start 
          ${darkMode === "light" ? "  bg-white text-[#828FA3]" : " bg-[#20212C] text-[#828FA3]"}`}
            ...
            >
              {boards?.[boardIndex].columns.map((column, index) => (
                <motion.button
                  className={`${darkMode === "light" ? "hover:text-black " : "hover:text-white"}   ${
