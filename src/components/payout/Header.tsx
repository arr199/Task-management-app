...
export function Header ({ boards, setShowNewTask, setShowEditBoardForm }: HeaderProps): JSX.Element {
  ...
  useEffect(() => {
    ...
    function handleClickOutside (e: MouseEvent): void {
      ...
      if (container !== null && !container?.contains(target) && target.id !== '3dots-menu' && target.id !== '3dots-icon') {
        setOpen(false)
  ...
  function handleLogOut (): void {
    ...
    signOut(auth).then().catch(err => { console.log('Error : ', err) })
  }
  ...
  return (
    <>
      <header className={` relative flex transition-all duration-300  w-full border-b   py-11 md:px-10
        ...
        <nav className="px-4 md:px-0   flex  w-full items-center gap-2 ">
          ...
          <div className="pl-1  sm:pl-[3vh]  md:min-w-[300px]  items-center ">
            ...
            <img className='md:flex hidden '
              src={`${darkMode === 'light' ? '/assets/logo-dark.svg' : '/assets/logo-light.svg'}`} alt='kanban logo' />
          </div>
          <div className='hidden md:flex absolute ml-[239px]  border-r-[#4b4b4b] border-r h-full'></div>
          {/* BOARD TITLE */}

          <h2 className="whitespace-nowrap pl-2  text-[18px] md:text-[24px] font-bold overflow-hidden text-ellipsis max-w-[40%]" >{boards?.[Number(board)]?.name}</h2>
          {/* ARROW BUTTON SMALL SCREEN */}
          <img onClick={handleShowLeftNavbar} width="15" className='md:hidden' src={`${showLeftNavbar ? 'assets/icon-chevron-up.svg' : 'assets/icon-chevron-down.svg'} `} ></img>

          {/* ADD NEW TASK */}
          <button data-id="add-new-task" disabled={boards?.length <= 0 || !boards[Number(board)]} onClick={handleShowAddTask}
           ...
           ${darkMode === 'light' ? 'text-white' : ''}`}><FaPlus ></FaPlus> <span className='hidden sm:flex'>Add New Task</span></button>
          {/* SHOW EDIT AND DELETE MENU POPUP */}
          <button id='3dots-menu' disabled={boards?.length <= 0 || !boards[Number(board)]} onClick={handleOpenMenu}>
            <BiDotsVerticalRounded id="3dots-icon" className='w-8 h-8 text-[#828FA3]  ' /> </button>
          {/* LOG OUT */}
          <button data-id="logout-button" onClick={handleLogOut} className='hidden  sm:flex px-2 hover:text-[#7445b3]'><IoMdLogOut className="w-6 h-6"></IoMdLogOut></button>
          {/* EDIT DELETE POP UP */}
          {open &&
            <div id='edit-delete-popup'
          ...
          ${darkMode === 'light' ? 'bg-white' : ''}`}>
              <button id='edit-board' onClick={handleEditBoard}
                className={`text-[#828FA3] e hover:font-bold 
            ${darkMode === 'light' ? ' hover:text-black ' : ' hover:text-white '}`}>Edit Board</button>
              <button id="delete-board" onClick={handleDeleteBoardPopUp} className='text-red-600 hover:text-red-400 hover:font-bold'>Delete Board</button>
              <button id="delete-board" onClick={handleLogOut} className='sm:hidden text-red-600 hover:text-red-400 hover:font-bold'>Log out!</button>
            </div>}
        </nav>
      </header>
